...
begin Pages:
    method handle(request: Request, next: MiddlewareNext): Response =
            ...
            if os.fileExists(config):
                ...
                for branch, matcher in matchers.getTable():
                    ...
                    if matcher.contains("pattern"):
                        match = segment.match(re matcher["pattern"].getStr())
                    else:
                        raise newException(
                            ValueError,
                    ...
                    if isSome(match):
                        ...
                        if matcher.contains("mapping"):
                            ...
                            if mapping.len != captures.len:
                                raise newException(
                                    ValueError,
                                    fmt "Cannot map pattern in file '{config}', {mapping.len}/{captures.len} matches are mapped"
                                )
        ...
        when defined debug:
            echo fmt "Matched template path '{path}'"
